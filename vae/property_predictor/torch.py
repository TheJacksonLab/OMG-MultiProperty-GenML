...
class PropertyNetworkPredictionModule(nn.Module):
    def __init__(self, latent_dim, property_dim, property_network_hidden_dim_list, dtype, weights=(0.5, 0.5),
                 property_linear=False):
        ...
        if property_dim >= 1:
            for dim_list in property_network_hidden_dim_list:
                if not property_linear:
                    ...
                    if len(dim_list) != 0:
                        raise ValueError('Linear model should have no hidden layer!')
                    property_network = nn.Linear(in_features=latent_dim, out_features=1)
