...
def optimize_qnehvi_and_get_observation(model, ref_point, train_z_tensor, qnehvi_sampler, inequality_constraints):
    ...
    targeted_re_encoding_property_prediction_arr = np.hstack([monomer_phi, lumo_energy - homo_energy, chi_water])
...
if __name__ == '__main__':
    """
    This script implements Bayesian optimization.         
    """
    ####### MODIFY #######
    # load vae parameters
    ...
    initial_train_y = np.load(os.path.join(initial_train_dir, 'sampled_properties.npy'))  # (num_data, 3)

    # valid data - unscaled
    test_z = np.load(os.path.join(gpr_dir, 'valid_z_mean.npy'))[:100]
    test_y = np.load(os.path.join(gpr_dir, 'valid_property.npy'))[:100]  # (num_data, 3)
    ...
    reference_polymer = '*OCCN(CCOC(=O)C(CCCc1ccsc1)C(*)=O)c1ccccn1'
    latent_dim = initial_train_z.shape[1]

    # reference_point = np.zeros(latent_dim).reshape(1, -1)
    # chosen_latent_space_points_arr, valid_polymer_list = generated_polymers(
    #     chosen_latent_space_points_arr=reference_point,
    #     vae_decoder=vae_decoder,
    #     grammar_fragment=grammar_fragment,
    #     device=device,
    #     dtype=dtype
    # )
    # assert len(valid_polymer_list) == 1
    ...
    # np.save(os.path.join(gpr_dir, 'reference_mu_arr.npy'), reference_mu_arr)
    ...
    targeted_reference_re_encoding_property_prediction_arr = np.array([
        monomer_phi, lumo_energy - homo_energy, chi_water
    ])
